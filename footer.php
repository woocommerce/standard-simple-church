				...
				<div class="row">
					...
					<div id="credit" class="<?php echo has_nav_menu( 'footer_menu' ) && ! standard_is_offline() ? 'span5' : 'span12'; ?>">
						...
						<?php if( null != get_page_by_path( 'privacy-policy' ) && 0 != get_page_by_path( 'privacy-policy' )->ID && 'publish' == get_page_by_path( 'privacy-policy' )->post_status ) { ?>							
							<?php printf( __( '&copy; %1$s %2$s &mdash; %3$s ', 'standard' ), date( 'Y' ), '<a href="' . site_url() . '">' . get_bloginfo( 'name' ) . '</a>', '<a href="' . get_permalink( get_page_by_path( 'privacy-policy' )->ID ) . '">Privacy Policy</a>') ?>
						<?php } else { ?>
							<?php printf( __( '&copy; %1$s %2$s', 'standard' ), date( 'Y' ), '<a href="' . site_url() . '">' . get_bloginfo( 'name' ) . '</a>' ) ?>
						<?php } // end if/else ?>
